...
mod tests {
    ...
    /// Pins pi `src/auth/oauth/oauth-page.ts:3-10` and `:96-108`.
    #[test]
    fn escapes_all_dynamic_html_fields() {
        let html = oauth_error_html("<&\"'>", Some("<script>alert('&')</script>"));
        assert!(html.contains("&lt;&amp;&quot;&#39;&gt;"));
        assert!(html.contains("&lt;script&gt;alert(&#39;&amp;&#39;)&lt;/script&gt;"));
        assert!(!html.contains("<script>"));
        assert!(!oauth_error_html("failed", Some("")).contains("class=\"details\""));
    }
}
