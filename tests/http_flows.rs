...
use rust_genai_auth::codex::{CodexAuth, CodexConfig};
use rust_genai_auth::{CredentialStore, FileCredentialStore, OAuthCredential};
...
async fn authorization_code_exchange_builds_correct_request() {
    ...
    let before = rust_genai_auth::credential::now_unix_ms();
    let cred = auth
    ...
    let after = rust_genai_auth::credential::now_unix_ms();
    ...
    assert_eq!(
        ...
        Some(rust_genai_auth::codex::CLIENT_ID)
    );
...
async fn token_exchange_surfaces_http_error() {
    ...
    let err = auth
        ...
        .unwrap_err();
    match err {
        rust_genai_auth::Error::TokenRequest {
            operation,
            status,
            body,
        } => {
...
async fn missing_account_id_is_rejected_by_default() {
    ...
    let err = auth
        ...
        .unwrap_err();
    assert!(
        matches!(err, rust_genai_auth::Error::MissingAccountId),
        "got {err:?}"
    );
}
...
async fn refresh_builds_correct_request_and_updates_store() {
    ...
    store
        .store(rust_genai_auth::OPENAI_CODEX_PROVIDER_ID, &refreshed)
        .unwrap();
...
async fn refresh_without_refresh_token_errors() {
    ...
    assert!(
        matches!(err, rust_genai_auth::Error::MissingRefreshToken),
        "got {err:?}"
    );
...
async fn device_code_polls_until_authorized() {
    ...
    assert_eq!(
        ...
        rust_genai_auth::codex::CLIENT_ID
    );
...
async fn device_code_not_enabled_maps_404() {
    ...
    assert!(
        matches!(err, rust_genai_auth::Error::DeviceCodeNotEnabled),
        "got {err:?}"
    );
}
...
async fn complete_browser_login_validates_state_and_exchanges() {
    ...
    let bad = auth
        ...
        .unwrap_err();
    assert!(
        matches!(bad, rust_genai_auth::Error::StateMismatch),
        "got {bad:?}"
    );
    assert!(server.requests().is_empty());
...
async fn genai_resolver_refreshes_and_persists() {
    ...
    let _resolver = rust_genai_auth::genai_integration::codex_auth_resolver(
        auth.clone(),
        store.clone(),
    ...
    let token = rust_genai_auth::genai_integration::resolve_access_token(
        &auth,
        store.as_ref(),
        rust_genai_auth::OPENAI_CODEX_PROVIDER_ID,
