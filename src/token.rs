...
//!
//! [`CodexStreamFn`](crate::CodexStreamFn) resolves a fresh token **before every
...
/// [`CodexStreamFn`](crate::CodexStreamFn) to emit an in-band terminal error
...
/// `rust_genai_auth::jwt::extract_chatgpt_account_id`, mirroring pi's
/// `extractAccountId(apiKey)` (openai-codex-responses.ts:1579-1590).
#[cfg(feature = "auth-resolver")]
pub struct ResolverTokenSource {
    resolver: std::sync::Arc<rust_genai_auth::genai_integration::CodexTokenResolver>,
}

#[cfg(feature = "auth-resolver")]
impl ResolverTokenSource {
    /// Wrap a shared [`CodexTokenResolver`](rust_genai_auth::genai_integration::CodexTokenResolver).
    pub fn new(
        resolver: std::sync::Arc<rust_genai_auth::genai_integration::CodexTokenResolver>,
    ) -> Self {
        Self { resolver }
...
impl TokenSource for ResolverTokenSource {
    async fn fetch(&self) -> Result<CodexToken> {
        ...
        let account_id = rust_genai_auth::jwt::extract_chatgpt_account_id(&bearer)
