...
fn legacy_context(context: &agentprism_ai::Context) -> Result<OrderedJsonObject, EncodeError> {
...
fn legacy_assistant_content(message: &agentprism_ai::AssistantMessage) -> OrderedJsonArray {
    let mut result = OrderedJsonArray::new();
            ...
            ContentBlock::Thinking { text, redacted, .. } => {
                ...
                if *redacted
                    || replay_has_kind(
                        message,
                        block,
                        agentprism_ai::PI_MESSAGES_VISIBLE_THINKING_KIND,
                    )
                {
...
fn signature_kind_matches(block: &ContentBlock, kind: &str) -> bool {
    match block {
        ContentBlock::Text { .. } => matches!(
            kind,
            agentprism_ai::GOOGLE_THOUGHT_SIGNATURE_KIND
                | agentprism_ai::OPENAI_RESPONSES_MESSAGE_IDENTITY_KIND
                | agentprism_ai::PI_MESSAGES_TEXT_SIGNATURE_KIND
        ),
        ContentBlock::Thinking { .. } => {
            kind != agentprism_ai::OPENAI_RESPONSES_MESSAGE_IDENTITY_KIND
                && kind != agentprism_ai::OPENAI_RESPONSES_FUNCTION_CALL_IDENTITY_KIND
                && kind != agentprism_ai::PI_MESSAGES_REDACTED_THINKING_KIND
                && kind != agentprism_ai::PI_MESSAGES_VISIBLE_THINKING_KIND
        }
        ContentBlock::ToolCall { .. } => matches!(
            kind,
            agentprism_ai::GOOGLE_THOUGHT_SIGNATURE_KIND
                | agentprism_ai::OPENAI_CHAT_REASONING_DETAIL_KIND
        ),
...
fn legacy_usage(
    usage: &agentprism_ai::Usage,
    cost: Option<&agentprism_ai::Cost>,
) -> OrderedJsonObject {
    let mut wire = OrderedJsonObject::new();
