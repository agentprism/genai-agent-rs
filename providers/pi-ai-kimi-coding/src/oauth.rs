...
impl KimiCodingOAuth {
    ...
    pub fn with_host(transport: Arc<dyn pi_ai::HttpTransport>, host: Url) -> Self {
        Self { transport, host }
    }
...
impl OAuthAuth for KimiCodingOAuth {
    ...
    fn refresh(
        ...
        Box::pin(async move {
            ...
            for attempt in 0..=3 {
                if attempt > 0 {
                    pi_ai::OAuthDeviceCodeRuntime::sleep(
                        &pi_ai::SystemOAuthDeviceCodeRuntime,
                ...
                let (status, _, body) = match pi_ai_provider_common::execute_send(
...
impl LocalOAuthAuth for LocalKimiCodingOAuth {
    ...
    fn login(
        &self,
        interaction: Rc<dyn pi_ai::LocalAuthInteraction>,
        cancellation: CancellationToken,
    ...
    fn refresh(
        ...
        Box::pin(async move {
            ...
            for attempt in 0..=3 {
                ...
                let (status, _, body) = match pi_ai_provider_common::execute_local(
                    self.transport.as_ref(),
                    request,
                    cancellation.clone(),
                )
...
fn device_token_request(url: &Url, device_code: &str) -> Result<pi_ai::HttpRequest, AuthError> {
...
impl DeviceNotifier for dyn pi_ai::LocalAuthInteraction {
    fn notify_device(&self, event: AuthEvent) -> Result<(), AuthError> {
        self.notify(event).map_err(AuthError::from)
    }
}
