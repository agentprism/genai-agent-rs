...
impl GitHubCopilotOAuth {
    ...
    pub fn new(transport: Arc<dyn pi_ai::HttpTransport>) -> Self {
        Self { transport }
    }
}
...
async fn fetch_models_local(
    ...
    for retry in 0..=max_retries {
        ...
        pi_ai::LocalOAuthDeviceCodeRuntime::sleep(
...
async fn execute_copilot_send(
    ...
    let execution = Box::pin(pi_ai_provider_common::execute_send(
        transport,
        request,
...
async fn execute_copilot_local(
    ...
    let timeout = effective_request_timeout(&request, retry_budget_remaining)?;
    let request_cancellation = cancellation.child();
    let execution = Box::pin(pi_ai_provider_common::execute_local(
        transport,
        request,
        request_cancellation.clone(),
    ));
...
fn github_form(url: Url, fields: &[(&str, &str)]) -> Result<pi_ai::HttpRequest, AuthError> {
    let mut request = pi_ai_provider_common::form_post(url.as_str(), fields)?;
    request
        .headers
...
fn bearer_get(url: Url, token: &str, models: bool) -> Result<pi_ai::HttpRequest, AuthError> {
...
fn policy_request(url: Url, token: &str) -> Result<pi_ai::HttpRequest, AuthError> {
    let mut headers = copilot_headers();
    headers.insert(
        header::AUTHORIZATION,
        HeaderValue::from_str(&format!("Bearer {token}"))
...
mod tests {
    ...
    impl pi_ai::HttpTransport for StalledTransport {
        fn execute(
            &self,
    ...
    fn short_deadline_request() -> pi_ai::HttpRequest {
