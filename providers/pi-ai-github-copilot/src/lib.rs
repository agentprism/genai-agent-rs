...
fn load_catalog() -> Result<Vec<ModelDescriptor>, String> {
    ...
    for api in ["openai-completions", "openai-responses"] {
        models.extend(
            pi_ai_openai::parse_openai_published_catalog(source, "github-copilot", api)
                .map_err(|error| error.to_string())?,
        );
    }
    Ok(models)
...
fn has_copilot_vision_input(context: &Context) -> bool {
    context.messages.iter().any(|message| match message {
        Message::User(message) => message
            .content
            .iter()
            .any(|block| matches!(block, pi_ai::ContentBlock::Image { .. })),
...
pub fn provider(inputs: ProviderInputs) -> Result<pi_ai::ProviderRegistration, ProviderBuildError> {
    ...
    pi_ai::ProviderRegistration::builder("github-copilot")
        .display_name("GitHub Copilot")
        .auth(pi_ai_provider_common::bearer_auth(
        ...
        .api(
            ...
            copilot_api(pi_ai_anthropic::anthropic_messages_api(Arc::clone(
                &inputs.http,
            ))),
        )
        .api(
            ApiId::new("openai-completions"),
            copilot_api(pi_ai_openai::openai_completions_api(Arc::clone(
                &inputs.http,
            ))),
        )
        .api(
            ApiId::new("openai-responses"),
            copilot_api(pi_ai_openai::openai_responses_api(inputs.http)),
        )
        .build()
...
pub fn local_provider(
    ...
    pi_ai::LocalProviderRegistration::builder("github-copilot")
        .display_name("GitHub Copilot")
        .auth(pi_ai_provider_common::local_bearer_auth(
            "GitHub Copilot token",
            "COPILOT_GITHUB_TOKEN",
            Some(oauth),
        ))
        ...
        .api(
            ApiId::new("anthropic-messages"),
            local_copilot_api(pi_ai_anthropic::local_anthropic_messages_api(Rc::clone(
                &inputs.http,
            ))),
        )
        .api(
            ApiId::new("openai-completions"),
            local_copilot_api(pi_ai_openai::local_openai_completions_api(Rc::clone(
                &inputs.http,
            ))),
        )
        .api(
            ApiId::new("openai-responses"),
            local_copilot_api(pi_ai_openai::local_openai_responses_api(inputs.http)),
        )
        .build()
        .map_err(ProviderBuildError::Registration)
}
