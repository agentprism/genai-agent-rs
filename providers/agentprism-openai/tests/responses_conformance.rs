...
fn responses_session_affinity_headers_match_pi() {
    let compat = OpenAiResponsesCompat {
        session_affinity_format: Some(agentprism_ai::SessionAffinityFormat::OpenAi),
        ..Default::default()
    };
    let mut headers = HeaderMap::new();
    agentprism_ai::apply_openai_responses_full_headers(
        &compat,
        &OpenAiResponsesOptions {
            cache_retention: CacheRetention::Short,
            session_id: Some("session-1".into()),
...
fn responses_codex_session_affinity_headers_match_pi() {
    ...
    assert_eq!(
        agentprism_ai::openai_codex_responses_transport_session_id(
    ...
    let mut headers = HeaderMap::new();
    agentprism_ai::apply_openai_codex_responses_full_headers(
        &OpenAiCodexResponsesOptions {
            temperature: None,
            reasoning_effort: None,
...
fn responses_compat_defaults_and_overrides_match_pi() {
    ...
    assert_eq!(
        ...
        Some(agentprism_ai::SessionAffinityFormat::OpenRouter)
    );
}
...
fn captured_responses_model(value: &Value, family: &str) -> ModelDescriptor {
    ...
    ModelDescriptor {
        common: CommonModelDescriptor {
            model_ref: agentprism_ai::ModelRef::new(
...
fn encode_captured_responses(
    ...
    if canonical["family"] == "openai-codex-responses" {
        ...
        let config =
            agentprism_openai::azure_model_config(&typed.config).expect("Azure fixture config");
        let compat =
        ...
        let options = if simple {
            ...
            AzureOpenAiResponses::lower_simple(
                ...
                &simple,
                &agentprism_openai::AzureOpenAiResponsesSimplePatch {
                    reasoning_summary: None,
...
fn captured_ordered_object(value: &Value) -> OrderedJsonObject {
    ...
    match agentprism_ai::parse_ordered_json(serde_json::to_vec(value).unwrap())
...
fn responses_model(provider: &str, api: &str, model: &str) -> ModelDescriptor {
    ...
    ModelDescriptor {
        common: CommonModelDescriptor {
            model_ref: agentprism_ai::ModelRef::new(provider, model),
...
fn priced_decode_context(api: &str, model: &str) -> OpenAiResponsesDecodeContext {
    ...
    context.pricing.default.input = agentprism_ai::MoneyRate::new(1_000_000);
    context.pricing.default.output = agentprism_ai::MoneyRate::new(2_000_000);
    context
}
