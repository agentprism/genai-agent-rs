...
fn openai_chat_partial_reasoning_detail_is_not_replayable() {
    let mut decoder = agentprism_openai::OpenAiCompletionsSseDecoder::new(decode_context());
...
fn openai_reasoning_format_matches_compat() {
    ...
    let mapped_options = lower_simple_options(
        ...
        &one_user_context(),
        &SimpleGenerationOptions {
            reasoning: Some(agentprism_ai::ReasoningLevel::High),
            ..Default::default()
        },
    );
    assert_eq!(
...
fn openrouter_reasoning_metadata_and_payloads_pi_exact() {
    ...
    let explicit = encode_options(
        ...
        lower_simple_options(
            ...
            &one_user_context(),
            &SimpleGenerationOptions {
                reasoning: Some(agentprism_ai::ReasoningLevel::Low),
                ..Default::default()
            },
        ),
    );
...
fn openai_thinking_budget_field_matches_compat() {
    ...
    for level in [
        agentprism_ai::ReasoningLevel::Xhigh,
        agentprism_ai::ReasoningLevel::Max,
    ] {
...
"#,
    ...
    let tool_block = events
        ...
        .find_map(|event| match event {
            AssistantEvent::ContentBlockStarted {
                block_id,
                kind: agentprism_ai::ContentBlockKind::ToolCall,
                ..
            } => Some(block_id.clone()),
            _ => None,
        })
...
fn openai_responses_header_only_auth_requires_explicit_options_send_and_local() {
    let mut model = agentprism_openai::openai_models().unwrap().remove(0);
    ...
    let local_model = agentprism_openai::openai_models().unwrap().remove(0);
...
impl AuthResolver for AbsentAuth {
    ...
    ) -> SendBoxFuture<'_, Result<Option<agentprism_ai::ResolvedAuth>, AuthError>> {
...
impl LocalAuthResolver for AbsentAuth {
    fn resolve(
        ...
        _cancellation: CancellationToken,
    ) -> LocalBoxFuture<'_, Result<Option<agentprism_ai::ResolvedAuth>, AuthError>> {
        Box::pin(async { Ok(None) })
    }
}
        ...
        if let Ok(Value::Array(details)) = serde_json::from_str::<Value>(signature) {
            for detail in details {
                ...
                replay.items.push(ReplayItem {
                    ...
                    payload: agentprism_ai::OpaquePayload::JsonBytes(ordered),
                });
            }
        } else {
            replay.items.push(ReplayItem {
                ...
                applicability: ReplayApplicability::ExactProviderApiModel,
                completeness: ReplayCompleteness::Complete,
                payload: agentprism_ai::OpaquePayload::Utf8(signature.into()),
            });
        }
    }
    AssistantMessage {
...
fn try_encode_options(
    ...
    context: &Context,
    options: OpenAiCompletionsOptions,
) -> Result<Vec<u8>, agentprism_ai::EncodeError> {
    let ApiModelConfig::OpenAiCompletions(config) = &model.api else {
        unreachable!()
    };
    let typed = TypedModelDescriptor::<OpenAiCompletions> {
    ...
    OrderedJsonWriter::to_vec(&wire.into()).map_err(|error| {
        agentprism_ai::EncodeError::InvalidRequest {
    ...
    AssistantMessage {
        ...
        replay: ReplayEnvelope {
            ...
            items: vec![ReplayItem {
                ...
                applicability: ReplayApplicability::ExactProviderApiModel,
                completeness: ReplayCompleteness::Complete,
                payload: agentprism_ai::OpaquePayload::JsonBytes(
