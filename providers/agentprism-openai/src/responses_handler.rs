...
) -> Option<(agentprism_ai::AssistantEvent, SendResponsesDecodeState)> {
    loop {
...
) -> Option<(agentprism_ai::AssistantEvent, LocalResponsesDecodeState)> {
    loop {
        if let Some(event) = state.pending.pop_front() {
            return Some((event, state));
        }
...
fn payload<A: ApiFamily<WireRequest = agentprism_ai::OrderedJsonObject>>(
    model: TypedModelDescriptor<A>,
    wire: agentprism_ai::OrderedJsonObject,
) -> Result<ProviderPayload, AiError> {
...
impl OpenAiCodexWebSocketRequest {
    ...
    ) -> Result<Vec<u8>, agentprism_ai::TransportError> {
...
pub trait OpenAiCodexWebSocketTransport: Send + Sync + 'static {
    ...
    fn execute(
        ...
        cancellation: CancellationToken,
    ) -> SendBoxFuture<'_, Result<OpenAiCodexWebSocketResponse, agentprism_ai::TransportError>>;
}
...
pub trait LocalOpenAiCodexWebSocketTransport: 'static {
    ...
    fn execute(
        ...
        request: OpenAiCodexWebSocketRequest,
        cancellation: CancellationToken,
    ) -> LocalBoxFuture<'_, Result<LocalOpenAiCodexWebSocketResponse, agentprism_ai::TransportError>>;
}
...
impl OpenAiCodexResponsesTransport {
    async fn execute_selected(
        ...
        request: HttpRequest,
        cancellation: CancellationToken,
    ) -> Result<HttpResponse, agentprism_ai::TransportError> {
...
fn codex_account_id(request: &HttpRequest) -> Result<String, agentprism_ai::TransportError> {
    request
        .auth_headers
...
fn codex_request_json(body: &[u8]) -> Result<serde_json::Value, agentprism_ai::TransportError> {
...
fn semantic_frame_terminates_adapted_body(
    item: &Result<Vec<u8>, agentprism_ai::TransportError>,
) -> bool {
    ...
    Box::pin(stream::unfold(
        ...
        |(
            ...
            let item = match item {
                ...
                None => {
                    let error = agentprism_ai::TransportError::new(
...
async fn execute_codex_websocket_send(
    ...
    let session_id = codex_session_id(&request);
    let websocket_request_id = agentprism_ai::clamp_openai_prompt_cache_key(session_id.as_deref())
...
async fn execute_codex_websocket_local(
    ...
    cached: bool,
    cancellation: CancellationToken,
) -> Result<LocalHttpResponse, agentprism_ai::TransportError> {
    let configured_transport = request.transport.unwrap_or_default();
    let request_bytes = request.body.len();
    let account_id = codex_account_id(&request)?;
    let session_id = codex_session_id(&request);
...
async fn next_codex_websocket_frame_send(
...
) -> Result<Option<Vec<u8>>, agentprism_ai::TransportError> {
    if cancellation.is_cancelled() {
        return Err(agentprism_ai::TransportError::new(
            "cancelled",
    ...
    futures_util::select_biased! {
        _ = cancelled => Err(agentprism_ai::TransportError::new(
            "cancelled",
            "Codex WebSocket request was cancelled",
        )),
        _ = timeout => Err(agentprism_ai::TransportError::new(
...
fn codex_fallback_diagnostic(
    ...
    request: &HttpRequest,
    error: &agentprism_ai::TransportError,
...
fn reassert_codex_session_headers(headers: &mut HeaderMap, session_id: Option<&str>) {
    let Some(session_id) = agentprism_ai::clamp_openai_prompt_cache_key(session_id)
