...
fn device_request() -> Result<pi_ai::HttpRequest, AuthError> {
...
fn device_token_request(device_code: &str) -> Result<pi_ai::HttpRequest, AuthError> {
    pi_ai_provider_common::form_post(
        TOKEN_URL,
        &[
...
fn refresh_request(refresh: &str) -> Result<pi_ai::HttpRequest, AuthError> {
    pi_ai_provider_common::form_post(
        TOKEN_URL,
        &[
...
fn notify_local(
    interaction: &dyn pi_ai::LocalAuthInteraction,
    device: &Device,
) -> Result<(), AuthError> {
    interaction.notify(event(device)).map_err(AuthError::from)
}
