...
fn parse_thinking_levels(value: &Value) -> ThinkingLevelMap<agentprism_ai::AnthropicThinkingValue> {
    let parse = |name: &str| {
        value.get(name).map(|value| match value.as_str() {
            None => LevelSupport::Unsupported,
            Some("off") => LevelSupport::Value(agentprism_ai::AnthropicThinkingValue::Off),
            Some("minimal") => LevelSupport::Value(agentprism_ai::AnthropicThinkingValue::Effort(
                AnthropicEffort::Minimal,
            )),
            Some("low") => LevelSupport::Value(agentprism_ai::AnthropicThinkingValue::Effort(
                AnthropicEffort::Low,
            )),
            Some("medium") => LevelSupport::Value(agentprism_ai::AnthropicThinkingValue::Effort(
                AnthropicEffort::Medium,
            )),
            Some("high") => LevelSupport::Value(agentprism_ai::AnthropicThinkingValue::Effort(
                AnthropicEffort::High,
            )),
            Some("xhigh") => LevelSupport::Value(agentprism_ai::AnthropicThinkingValue::Effort(
                AnthropicEffort::Xhigh,
            )),
            Some("max") => LevelSupport::Value(agentprism_ai::AnthropicThinkingValue::Effort(
                AnthropicEffort::Max,
            )),
...
fn parse_assistant(
            ...
            Some("thinking") => {
                ...
                if let Some(signature) = block["thinkingSignature"].as_str() {
                    ...
                    replay.items.push(ReplayItem {
                        ...
                        payload: agentprism_ai::OpaquePayload::Utf8(signature.to_owned()),
                    });
    ...
    AssistantMessage {
        ...
        finish: AssistantFinish {
            ...
            .then(|| agentprism_ai::PublicError {
...
fn anthropic_user_text_block_array_preserves_blocks_pi_exact() {
    ...
    context.messages = vec![Message::User(agentprism_ai::UserMessage {
...
fn anthropic_provider_registration_matches_pinned_catalog() {
    ...
    assert!(
        registration
            .apis
            .contains_key(&agentprism_ai::ApiId::new("anthropic-messages"))
    );
}
...
fn anthropic_auth_token_precedence_matches_pi() {
    ...
    let mut request =
        agentprism_ai::ResolveAuthRequest::isolated(registration.descriptor.clone(), None);
...
fn anthropic_oauth_tool_names_and_identity_round_trip() {
    ...
    let context = Context {
        ...
        messages: vec![Message::User(agentprism_ai::UserMessage {
...
fn anthropic_lowering_precedes_projection_for_output_cap() {
    ...
    model.common.limits.context_window = original_estimate
        .tokens
        .saturating_add(agentprism_ai::CONTEXT_SAFETY_TOKENS)
...
fn anthropic_zero_context_window_preserves_output_cap() {
    ...
    config.thinking_levels.low = Some(LevelSupport::Value(
        agentprism_ai::AnthropicThinkingValue::Budget(2_048),
    ));
...
fn anthropic_unknown_content_block_does_not_consume_content_index() {
    ...
    assert!(events.iter().any(|event| matches!(
        ...
        AssistantEvent::ContentBlockStarted {
            content_index: 0,
            kind: agentprism_ai::ContentBlockKind::Text,
            ..
        }
    )));
    let message = terminal_message(&events);
