        ...
        Box::pin(async move {
            let body = match reply {
                WebSocketReply::Frames(frames) => {
                    Ok(Box::pin(stream::iter(frames.into_iter().map(Ok))) as pi_ai::HttpBody)
                }
                ...
                WebSocketReply::FramesThenPending(frames) => Ok(Box::pin(
                    ...
                    as pi_ai::HttpBody),
                WebSocketReply::Pending => Ok(Box::pin(stream::pending()) as pi_ai::HttpBody),
...
impl LocalOpenAiCodexWebSocketTransport for LocalRecordingWebSocket {
    fn execute(
        ...
        Box::pin(async move {
            let body = match reply {
                WebSocketReply::Frames(frames) => {
                    Ok(Box::pin(stream::iter(frames.into_iter().map(Ok))) as pi_ai::LocalHttpBody)
                }
                WebSocketReply::FramesThenError(frames, code) => Ok(Box::pin(
                    stream::iter(frames.into_iter().map(Ok)).chain(stream::once(async move {
                        Err(TransportError::new(code, code))
                    ...
                    as pi_ai::LocalHttpBody),
                WebSocketReply::FramesThenPending(frames) => Ok(Box::pin(
                    stream::iter(frames.into_iter().map(Ok)).chain(stream::pending()),
                )
                    as pi_ai::LocalHttpBody),
                WebSocketReply::Pending => Ok(Box::pin(stream::pending()) as pi_ai::LocalHttpBody),
                WebSocketReply::Error(code) => Err(TransportError::new(code, code)),
            }?;
...
fn codex_affinity_request(model: &pi_ai::ModelDescriptor) -> ModelRequest {
