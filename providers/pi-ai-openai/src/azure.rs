...
pub struct AzureOpenAiResponsesSimplePatch {
    ...
    pub reasoning_summary: Option<pi_ai::OpenAiResponsesReasoningSummary>,
...
impl ApiFamily for AzureOpenAiResponses {
    ...
    fn lower_simple(
        ...
        let config = azure_model_config(&context.model.config)
            .map_err(|message| pi_ai::LoweringError::InvalidConfiguration { message })?;
    ...
    fn encode(
        ...
        for message in &mut projected.messages {
            ...
            {
                assistant.replay.source.api = pi_ai::ApiId::new(OpenAiResponses::API_ID);
                assistant.api = pi_ai::ApiId::new(OpenAiResponses::API_ID);
            }
        }
        ...
        let prompt_cache_key =
            pi_ai::clamp_openai_prompt_cache_key(options.responses.session_id.as_deref());
