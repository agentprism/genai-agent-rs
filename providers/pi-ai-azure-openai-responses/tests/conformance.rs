...
use pi_ai::*;
use pi_ai_azure_openai_responses::*;
...
fn azure_openai_auth_options_and_environment_pi_exact_send_and_local() {
    ...
    assert_eq!(
        resolved.transport_headers[pi_ai_openai::AZURE_API_VERSION_AUTH_HEADER],
        "2026-06-01"
    );
    assert_eq!(
        resolved.transport_headers[pi_ai_openai::AZURE_DEPLOYMENT_MAP_AUTH_HEADER],
...
fn azure_deployment_map_truthiness_send_and_local() {
    ...
    assert_eq!(
        pi_ai_openai::parse_azure_deployment_name_map(
    ...
    let api = pi_ai_openai::azure_openai_responses_api(Arc::new(send.clone()));
    ...
    let api = pi_ai_openai::local_azure_openai_responses_api(Rc::new(local.clone()));
    let result = futures_executor::block_on(
