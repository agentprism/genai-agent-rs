...
pub fn provider(inputs: ProviderInputs) -> Result<pi_ai::ProviderRegistration, ProviderBuildError> {
    pi_ai_provider_common::build_provider(
        ...
        [(
            ...
            pi_ai_openai::azure_openai_responses_api(inputs.http) as Arc<dyn pi_ai::ChatApi>,
        )],
    )
}
...
pub fn local_provider(
    ...
    pi_ai_provider_common::build_local_provider(
        ...
        [(
            ApiId::new("azure-openai-responses"),
            pi_ai_openai::local_azure_openai_responses_api(inputs.http)
...
impl ApiKeyAuth for AzureApiKeyAuth {
    ...
    fn login(
        ...
        interaction: Arc<dyn AuthInteraction>,
        cancellation: CancellationToken,
    ) -> SendBoxFuture<'_, Result<pi_ai::ApiKeyCredential, AuthError>> {
...
impl LocalApiKeyAuth for AzureApiKeyAuth {
    ...
    fn login(
        ...
        interaction: Rc<dyn LocalAuthInteraction>,
        cancellation: CancellationToken,
    ) -> LocalBoxFuture<'_, Result<pi_ai::ApiKeyCredential, AuthError>> {
...
async fn prompt_key_send(
    ...
    cancellation: CancellationToken,
) -> Result<pi_ai::ApiKeyCredential, AuthError> {
    let answer = interaction
        .prompt(
            pi_ai::AuthPrompt::Secret {
...
fn credential_from_answer(answer: pi_ai::AuthAnswer) -> Result<pi_ai::ApiKeyCredential, AuthError> {
    let pi_ai::AuthAnswer::Text(key) = answer else {
        return Err(AuthError::new(
            "invalid_auth_answer",
    ...
    Ok(pi_ai::ApiKeyCredential {
        key: Some(pi_ai::SecretString::new(key)),
...
async fn augment_azure_auth_local(
    ...
    explicit: &std::collections::BTreeMap<String, String>,
    stored: &std::collections::BTreeMap<String, String>,
    context: &dyn pi_ai::LocalAuthContext,
    cancellation: CancellationToken,
) -> Result<(), AuthError> {
...
fn finish_azure_auth(
    ...
    if let Some(value) = api_version {
        insert_private_header(
            ...
            pi_ai_openai::AZURE_API_VERSION_AUTH_HEADER,
            &value,
        )?;
    }
    if let Some(value) = deployment_map {
        insert_private_header(
            &mut resolved.transport_headers,
            pi_ai_openai::AZURE_DEPLOYMENT_MAP_AUTH_HEADER,
            &value,
        )?;
    }
    Ok(())
...
fn azure_base_url(
    ...
    value
        .map(|value| {
            pi_ai_openai::normalize_azure_openai_base_url(&value).map_err(|error| {
                AuthError::new(
