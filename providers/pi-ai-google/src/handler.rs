...
trait GoogleFamily:
    ApiFamily<
        Compat = GoogleCompat,
        ModelConfig = pi_ai::GoogleModelConfig,
        OptionsPatch = GoogleSimplePatch,
        WireRequest = OrderedJsonObject,
    >
{
    fn config(model: &ModelDescriptor) -> Option<&pi_ai::GoogleModelConfig>;
...
fn decode_transport_route(route: &str) -> Result<GoogleTransportRoute, pi_ai::TransportError> {
...
fn generate_content_url(
    ...
    kind: GoogleApiKind,
) -> Result<Url, pi_ai::TransportError> {
...
fn validate_google_model(model: &str) -> Result<(), pi_ai::TransportError> {
    if model.is_empty() {
        return Err(pi_ai::TransportError::new(
