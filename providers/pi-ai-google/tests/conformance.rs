...
fn parse_assistant(value: &Value, index: usize, model: &ModelDescriptor) -> AssistantMessage {
    ...
    AssistantMessage {
        ...
        finish: AssistantFinish {
            ...
            error: (reason == AssistantFinishReason::Error).then(|| pi_ai::PublicError {
...
fn google_tool_call_signature_stays_on_function_call() {
    let events = decode_google_sse(
        ...
        GoogleDecodeContext {
            ...
            api: pi_ai::ApiId::new(GoogleGenerativeAi::API_ID),
    ...
    let replay_position = events
        .iter()
        .position(|event| matches!(event, pi_ai::AssistantEvent::ReplayItemStarted { .. }))
        .expect("replay start");
    let metadata_position = events
        .iter()
        .position(|event| matches!(event, pi_ai::AssistantEvent::ToolCallMetadata { .. }))
        .expect("tool metadata");
    assert!(replay_position < metadata_position);
    let message = events
        .iter()
        .find_map(pi_ai::AssistantEvent::terminal_message)
        .expect("terminal message");
...
fn lower_google_options<A>(
    ...
    levels: ThinkingLevelMap<String>,
    simple: &SimpleGenerationOptions,
) -> Result<GoogleOptions, pi_ai::LoweringError>
where
    A: ApiFamily<
            Compat = GoogleCompat,
            ModelConfig = GoogleModelConfig,
...
fn google_thinking_level_map_pi_exact() {
    ...
    invalid_options.reasoning_fallback = pi_ai::ReasoningFallback::Strict;
    assert!(matches!(
        ...
        Err(pi_ai::LoweringError::UnsupportedReasoningLevel {
...
fn google_provider_total_tokens_drive_turn_two_context_planning_pi_exact() {
    ...
    assert_eq!(pi_ai::calculate_context_tokens(&message.usage), 20);
...
impl pi_ai::HttpTransport for NeverTransport {
    fn execute(
        &self,
        _request: pi_ai::HttpRequest,
        _cancellation: pi_ai::CancellationToken,
    ) -> pi_ai::SendBoxFuture<'_, Result<pi_ai::HttpResponse, pi_ai::TransportError>> {
...
impl AdcTransport {
    fn requests(&self) -> Vec<pi_ai::HttpRequest> {
        self.requests.lock().expect("ADC request lock").clone()
    }
}

impl pi_ai::HttpTransport for AdcTransport {
    fn execute(
        &self,
        request: pi_ai::HttpRequest,
        _cancellation: pi_ai::CancellationToken,
    ) -> pi_ai::SendBoxFuture<'_, Result<pi_ai::HttpResponse, pi_ai::TransportError>> {
        ...
        self.requests
            ...
            .push(request);
        Box::pin(async move {
            Ok(pi_ai::HttpResponse::from_bytes(
                200,
                http::HeaderMap::new(),
...
impl pi_ai::LocalHttpTransport for AdcTransport {
    fn execute(
        &self,
        request: pi_ai::HttpRequest,
        _cancellation: pi_ai::CancellationToken,
        ...
        self.requests
            ...
            .push(request);
        Box::pin(async move {
            Ok(pi_ai::LocalHttpResponse::from_bytes(
                200,
                http::HeaderMap::new(),
                if is_token_exchange {
                    br#"{"access_token":"fixture-adc-token","expires_in":3600,"token_type":"Bearer"}"#
...
impl pi_ai_google::VertexAdcCredentialAdapter for RecordingVertexAdcAdapter {
    fn resolve_access_token(
        ...
        Box::pin(async move {
            cancellation
                .check()
                .map_err(|_| pi_ai::AuthError::Cancelled)?;
            self.requests
                .lock()
                .expect("Vertex ADC adapter request lock")
                .push(request);
            Ok(pi_ai::SecretString::new("fixture-delegated-adc-token"))
        })
    }
}
...
impl pi_ai::HttpTransport for CapturingTransport {
    fn execute(
        &self,
        request: pi_ai::HttpRequest,
        _cancellation: pi_ai::CancellationToken,
...
impl pi_ai::LocalHttpTransport for CapturingTransport {
    fn execute(
        &self,
        request: pi_ai::HttpRequest,
        _cancellation: pi_ai::CancellationToken,
...
impl pi_ai::LocalHttpTransport for LocalNeverTransport {
    fn execute(
        &self,
        _request: pi_ai::HttpRequest,
        _cancellation: pi_ai::CancellationToken,
...
fn google_generative_ai_authorization_only_is_authentication_send_and_local() {
    ...
    let error = futures_executor::block_on(
        registration.apis[&pi_ai::ApiId::new(GoogleGenerativeAi::API_ID)]
            .stream(request, pi_ai::CancellationToken::new()),
    )
    .expect_err("Authorization-only Developer API request must fail");
    assert_eq!(error.kind, pi_ai::AiErrorKind::Authentication);
    assert!(
        send_transport
            .requests
            .lock()
...
fn assert_google_erased_tool_choice_patch(family: &str, api: &str, endpoint: &str) {
    ...
    let implementation = &registration.apis[&pi_ai::ApiId::new(api)];
    futures_executor::block_on(
        implementation.stream(make_request("any"), pi_ai::CancellationToken::new()),
    )
    ...
    assert_eq!(error.kind, pi_ai::AiErrorKind::InvalidRequest);
    assert_eq!(
        capture
            .requests
            .lock()
...
fn google_provider_catalogs_and_registrations_match_pinned_families() {
    ...
    assert!(
        google_registration
            .apis
            .contains_key(&pi_ai::ApiId::new(GoogleGenerativeAi::API_ID))
    );
    assert_eq!(
    ...
    assert!(
        vertex_registration
            .apis
            .contains_key(&pi_ai::ApiId::new(GoogleVertex::API_ID))
    );
    assert_eq!(
    ...
    let _stream = futures_executor::block_on(
        registration.apis[&pi_ai::ApiId::new(GoogleVertex::API_ID)]
            .stream(request, pi_ai::CancellationToken::new()),
    )
...
fn google_vertex_custom_standard_host_preserves_collection_scope_send_and_local() {
    ...
    futures_executor::block_on(
        registration.apis[&pi_ai::ApiId::new(GoogleVertex::API_ID)]
            .stream(make_send_request(), pi_ai::CancellationToken::new()),
    )
    ...
    futures_executor::block_on(pi_ai::LocalChatApi::stream(
        registration.apis[&pi_ai::ApiId::new(GoogleVertex::API_ID)].as_ref(),
        make_local_request(),
        pi_ai::CancellationToken::new(),
    ))
...
fn google_generative_ai_qualified_model_urls_pi_exact_send_and_local() {
    for (model_id, expected_path) in [
        ...
        futures_executor::block_on(pi_ai::LocalChatApi::stream(
            registration.apis[&pi_ai::ApiId::new(GoogleGenerativeAi::API_ID)].as_ref(),
            request,
            pi_ai::CancellationToken::new(),
        ))
...
fn google_qualified_gemini3_disabled_thinking_pi_exact_send_and_local() {
    ...
    futures_executor::block_on(
        registration.apis[&pi_ai::ApiId::new(GoogleGenerativeAi::API_ID)].stream(
...
fn google_invalid_sdk_model_syntax_rejected_pi_exact_send_and_local() {
    for model_id in ["", "models/../secret", "gemini?alt=json", "gemini&x=y"] {
        ...
        assert_eq!(error.kind, pi_ai::AiErrorKind::Transport);
        assert!(
            send_capture
                .requests
                .lock()
        ...
        let error = futures_executor::block_on(pi_ai::LocalChatApi::stream(
            registration.apis[&pi_ai::ApiId::new(GoogleGenerativeAi::API_ID)].as_ref(),
            request,
            pi_ai::CancellationToken::new(),
        ))
...
fn google_vertex_full_options_project_location_precedence_pi_exact_send_and_local() {
    ...
    let make_request = || {
        ...
        request.full_options = Some(pi_ai::ErasedApiFullOptions::new::<GoogleVertex>(
            options.clone(),
        ));
        request
    };
    ...
    futures_executor::block_on(
        registration.apis[&pi_ai::ApiId::new(GoogleVertex::API_ID)]
            .stream(make_request(), pi_ai::CancellationToken::new()),
    )
    ...
    let model_ref = google_vertex_models().expect("local Vertex catalog")[0]
        ...
        .model_ref
        .clone();
    let models = pi_ai::LocalModels::builder()
