...
fn encode_options(
    ...
    options: &agentprism_ai::MistralOptions,
    ...
    let session_id = (!suppress_affinity
        && options.cache_retention != Some(agentprism_ai::CacheRetention::None))
...
fn has_header_override(headers: &agentprism_ai::HeaderMapSpec, target: &str) -> bool {
...
fn decode_context(model: &agentprism_ai::ModelDescriptor) -> MistralConversationsDecodeContext {
...
fn mistral_timeout_error(timeout: Duration) -> agentprism_ai::TransportError {
