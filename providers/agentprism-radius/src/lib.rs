        ...
        .map(|model| {
            ...
            Ok(ModelDescriptor {
                ...
                api: ApiModelConfig::Custom(CustomApiModelConfig {
                    api: ApiId::new(agentprism_pi_messages::PiMessages::API_ID),
                    schema_version: 1,
...
async fn collect_send(
    mut body: agentprism_ai::HttpBody,
    cancellation: &CancellationToken,
...
async fn collect_local(
    mut body: agentprism_ai::LocalHttpBody,
    cancellation: &CancellationToken,
) -> Result<Vec<u8>, CatalogError> {
    let mut result = Vec::new();
    while let Some(chunk) = body.next().await {
    ...
    fn resolve(
        ...
        Box::pin(async move {
            let configuration_check =
                request.purpose == agentprism_ai::AuthResolutionPurpose::ConfigurationCheck;
            ...
            if retention.as_deref() == Some("long") {
                auth.headers.insert(
                    agentprism_pi_messages::PI_CACHE_RETENTION_AUTH_HEADER,
...
pub fn radius_provider(
    ...
    ProviderRegistration::builder("radius")
        ...
        .api(
            agentprism_pi_messages::PiMessages::API_ID,
            agentprism_pi_messages::pi_messages_api(transport),
        )
        .build()
...
pub fn local_radius_provider(
    ...
    LocalProviderRegistration::builder("radius")
        ...
        .api(
            agentprism_pi_messages::PiMessages::API_ID,
            agentprism_pi_messages::local_pi_messages_api(transport),
        )
        .build()
        .map_err(RadiusProviderError::Registration)
}
