...
fn encode_options(
    ...
    compatibility: pi_ai::BedrockCompat,
...
fn build_bedrock_http_request(
...
) -> Result<crate::BedrockSigningConfig, pi_ai::TransportError> {
    let config = signing_config_from_headers(&request.auth_headers)
        .map_err(|error| pi_ai::TransportError::new("bedrock_auth", error.to_string()))?;
    ...
    let model_id = command
        ...
        .ok_or_else(|| {
            pi_ai::TransportError::new("bedrock_serialize", "Bedrock command omits modelId")
        })?;
    ...
    let mut segments = request.url.path_segments_mut().map_err(|()| {
        pi_ai::TransportError::new("bedrock_endpoint", "Bedrock endpoint cannot be a base URL")
    })?;
    segments
        .pop_if_empty()
        .push("model")
