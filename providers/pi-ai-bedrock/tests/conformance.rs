...
fn typed(model: &ModelDescriptor) -> TypedModelDescriptor<BedrockConverseStream> {
    let pi_ai::ApiModelConfig::BedrockConverse(config) = &model.api else {
...
) -> Result<String, pi_ai::EncodeError> {
    let projected =
...
fn terminal(events: &[AssistantEvent]) -> pi_ai::AssistantMessage {
    events
        .iter()
...
fn bedrock_redacted_bytes_survive_json_round_trip() {
    ...
    let restored: pi_ai::AssistantMessage =
...
fn bedrock_turn_two_replays_redacted_content_bytes() {
    ...
    let assistant: pi_ai::AssistantMessage =
    ...
    let fragments = events
        ...
        .filter_map(|event| match event {
            AssistantEvent::ReplayData {
                operation: pi_ai::ReplayDataOperation::AppendUtf8(fragment),
                ..
...
fn decoded_tool_turn(model: &ModelDescriptor, arguments: &str) -> pi_ai::AssistantMessage {
    let mut decoder = BedrockConverseDecoder::new(BedrockDecodeContext {
...
fn bedrock_user_and_tool_result_images_use_pi_atob_pi_exact() {
    ...
    *data = "AQ\u{000b}ID".to_owned();
    assert!(matches!(
        ...
        Err(pi_ai::EncodeError::InvalidRequest { .. })
    ));
        ...
        BedrockConverseStream::encode(
            EncodeContext {
                ...
                compat: match &unsupported.api {
                    pi_ai::ApiModelConfig::BedrockConverse(config) => &config.compat,
                    _ => unreachable!(),
                },
...
impl BedrockSigner for BodyDiagnosticSigner {
    fn execute(
        ...
        Box::pin(async {
            let diagnostic = pi_ai::AssistantMessageDiagnostic {
                schema_version: pi_ai::ASSISTANT_MESSAGE_DIAGNOSTIC_SCHEMA_VERSION,
...
fn fixture_model(case: &str) -> ModelDescriptor {
    ...
    if case == "api-specific-compat-flags"
        && let pi_ai::ApiModelConfig::BedrockConverse(config) = &mut model.api
    {
...
fn fixture_response(model: &ModelDescriptor, kind: FixtureResponseKind) -> pi_ai::AssistantMessage {
...
fn failed_fixture_response(model: &ModelDescriptor) -> pi_ai::AssistantMessage {
    let mut decoder = BedrockConverseDecoder::new(BedrockDecodeContext {
...
fn decode_fixture_response_bytes(model: &ModelDescriptor, bytes: &[u8]) -> pi_ai::AssistantMessage {
    let mut decoder = BedrockConverseDecoder::new(BedrockDecodeContext {
...
fn bedrock_thinking_levels_pi_exact() {
    ...
    let pi_ai::ApiModelConfig::BedrockConverse(config) = &mut native_xhigh.api else {
        panic!("Bedrock config")
    };
    config.thinking_levels.xhigh = Some(pi_ai::LevelSupport::Value("high".to_owned()));
    let body = encode_command(
        &native_xhigh,
        &one_user_context(),
        &BedrockOptions {
...
fn send_start_failure(failure: BedrockProviderFailure) -> pi_ai::AiError {
    let model = model("us.anthropic.claude-opus-4-8");
...
fn local_start_failure(failure: BedrockProviderFailure) -> pi_ai::AiError {
    let model = model("us.anthropic.claude-opus-4-8");
...
fn send_body_failure(failure: BedrockProviderFailure) -> pi_ai::AssistantMessage {
    let model = model("us.anthropic.claude-opus-4-8");
...
fn local_body_failure(failure: BedrockProviderFailure) -> pi_ai::AssistantMessage {
    let model = model("us.anthropic.claude-opus-4-8");
...
fn bedrock_error_metadata_normalization_pi_exact() {
    ...
    for error in [
        ...
        assert_eq!(error.kind, pi_ai::AiErrorKind::ProviderRejected);
        assert_eq!(
            error.message,
    ...
    for error in [
        ...
        assert_eq!(error.kind, pi_ai::AiErrorKind::Authorization);
        assert_eq!(
            error.message,
