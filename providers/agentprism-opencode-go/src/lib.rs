//! OpenCode Go provider leaf crate.
#![deny(missing_docs)]
...
fn load_catalog() -> Result<Vec<agentprism_ai::ModelDescriptor>, String> {
    ...
    for api in ["openai-completions", "openai-responses"] {
        models.extend(
            agentprism_openai::parse_openai_published_catalog(source, "opencode-go", api)
                .map_err(|error| error.to_string())?,
        );
    }
    Ok(models)
...
agentprism_provider_common::define_static_provider! {
    id: "opencode-go", name: "OpenCode Go", auth_name: "OpenCode API key", env: "OPENCODE_API_KEY",
