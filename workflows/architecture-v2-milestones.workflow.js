...
const PI_ROOT = A.piRoot || "/home/vikash/pi-pin-e86823096";
const PI_AI = PI_ROOT + "/packages/ai";
const PI_AGENT = PI_ROOT + "/packages/agent";
const PIN = A.pin || "e86823096c5bad39e1ca282ec24bc5eb9bec745b";
...
const MILESTONES = {
  ...
  M7: {
    ...
    packages: [
      ...
      { id: "M7.1", title: "pi-agent-session: entry tree, lanes, operation records, storage traits, reducer, recovery, branching", crate: "crates/agentprism-session", read: "part 2 §7.2–7.6 in full; pi harness/session/types.ts, state.ts, session.ts, context.ts, memory.ts and their tests.", deliver: ["Every type and trait named; the eight reducer invariants; RecoveryDecision; in-memory storage."], accept: ["§10.10 Reducer and session tree tests pass with their exact names."] },
    ...
    packages: [
      {
        ...
        crate: "crates/agentprism-session",
        ...
        deliver: [
          ...
          "Delete crates/agentprism-compat-pi-jsonl (crate and workspace membership) per the owner ruling; flip packages/agent/test/harness/session/jsonl-codec.test.ts, jsonl-storage.test.ts, and jsonl.test.ts to deliberate-divergence with reason: owner ruling 2026-08-25 — no backward compatibility (no existing consumers); the native durable store ports the protocol semantics, not the v4 byte format.",
        ],
      ...
      {
        id: "M9.2",
        title: "Parity-manifest burn-down: eliminate every remaining planned mapping",
        crate: "parity, crates/*, providers/*",
        read: "parity/manifest.toml — every [[mapping]] with status planned; for each, the upstream test file at the pin, in full.",
        deliver: [
          "For every remaining planned mapping: read the upstream test file and either (a) name the existing Rust tests that give semantic parity, verifying scenario-by-scenario that they actually cover the file (port any missing scenarios as new named tests), or (b) record a deliberate-divergence with a reason grounded in §10.11 or an owner ruling. No mapping may remain planned.",
          "Milestone labels on past-milestone planned entries were mapping debt; this package pays it. The commitment-gates report depends on this being truthful — a mapping claimed semantic-parity whose Rust tests do not cover the upstream scenarios is a defect.",
        ],
        accept: ["parity/check.sh passes with zero planned mappings; the reviewer reads at least 8 upstream files and their mapped Rust tests side by side to confirm claimed coverage is real."],
      },
    ],
  },
  M10: {
