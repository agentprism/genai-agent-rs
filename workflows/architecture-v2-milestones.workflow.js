...
const MILESTONES = {
  ...
  M6: {
    ...
    packages: [
      ...
      { id: "M6.4", title: "Azure OpenAI Responses, Mistral Conversations, pi-messages families; Cloudflare, Radius, and every remaining provider; pi-ai-providers-all", crate: "providers/*", read: "part 2 §10.8 family list, §5.7 (Radius as a ModelCatalogSource); pi api/azure-openai-responses.ts, mistral-conversations.ts, pi-messages.ts, cloudflare*.ts, providers/*.ts, providers/all.ts, remaining auth/oauth/*.ts flows.", deliver: ["Every remaining family and provider as leaf crates; OAuth flows for each provider through the M3.3 contracts; pi-ai-providers-all aggregator; catalogs from pi's published data."], accept: ["Every wire_*_pi_exact suite in §10.8 passes for its captured corpus; the Wire gate and Replay gate are met."] },
    ],
  },
  M7: {
    title: "Milestone 7 — sessions, environment, Tokio runtime (part 2 §7.2–7.6, §7.10, §9.4)",
    packages: [
