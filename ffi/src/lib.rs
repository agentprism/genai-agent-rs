    ...
    async fn after(
        ...
        match result {
            Some(result) => {
                let details = match result.details_json {
                    Some(json) => Some(serde_json::from_str(&json).map_err(|err| {
                        agent::ToolHookError::new(format!(
                            "invalid details_json from host hook: {err}"
                        ))
                    })?),
                    None => None,
                };
...
impl agent::ShouldStopAfterTurn for ShouldStopAdapter {
    async fn should_stop(
        ...
        self.0
            .should_stop(ctx.into(), Arc::new(AgentCancelToken { inner: cancel }))
            .await
    }
}
...
impl agent::PrepareNextTurn for PrepareNextTurnAdapter {
    async fn prepare(
        ...
        let update = self
            .0
            .prepare(ctx.into(), Arc::new(AgentCancelToken { inner: cancel }))
            .await?;
        Some(agent::AgentLoopTurnUpdate {
            context: update
                .context
                .map(|context| context.into_core(original_tools)),
...
impl Agent {
    ...
    pub fn new_with_api_keys(setup: AgentSetup, api_keys: HashMap<String, String>) -> Arc<Self> {
        let auth_resolver =
            genai::resolver::AuthResolver::from_resolver_fn(move |model_iden: genai::ModelIden| {
                ...
                Ok(api_keys
                    .get(&kind)
                    .map(|key| genai::resolver::AuthData::from_single(key.clone())))
            });
        ...
        Arc::new(Self {
            inner: agent::AgentBuilder::new(setup.into())
    ...
    pub async fn wait_for_idle(&self) {
        let agent = self.inner.clone();
        let _ = RUNTIME
            .spawn(async move { agent.wait_for_idle().await })
            .await;
    }
    ...
    pub fn set_transform_context_hook(
        &self,
        hook: Option<Arc<dyn TransformContextHook>>,
    ) -> Result<(), AgentError> {
        self.inner
            .set_transform_context_object(
                hook.map(|h| {
                    Arc::new(TransformContextAdapter(h)) as Arc<dyn agent::TransformContext>
                }),
            )
    ...
    pub fn set_before_tool_call_hook(
        &self,
        hook: Option<Arc<dyn BeforeToolCallHook>>,
    ) -> Result<(), AgentError> {
        self.inner
            .set_before_tool_call_object(
                hook.map(|h| Arc::new(BeforeToolCallAdapter(h)) as Arc<dyn agent::BeforeToolCall>),
            )
            .map_err(AgentError::from)
    }

    /// Set (or clear) the after-tool-call observer hook.
    pub fn set_after_tool_call_hook(
        &self,
        hook: Option<Arc<dyn AfterToolCallHook>>,
    ) -> Result<(), AgentError> {
        self.inner
            .set_after_tool_call_object(
                hook.map(|h| Arc::new(AfterToolCallAdapter(h)) as Arc<dyn agent::AfterToolCall>),
            )
            .map_err(AgentError::from)
    }

    /// Set (or clear) the fallible before-tool-call hook.
    pub fn set_try_before_tool_call_hook(
        &self,
        hook: Option<Arc<dyn TryBeforeToolCallHook>>,
    ) -> Result<(), AgentError> {
        self.inner
            .set_try_before_tool_call_object(hook.map(|h| {
                Arc::new(TryBeforeToolCallAdapter(h)) as Arc<dyn agent::TryBeforeToolCall>
            }))
            .map_err(AgentError::from)
    }

    /// Set (or clear) the fallible after-tool-call hook.
    pub fn set_try_after_tool_call_hook(
        &self,
        hook: Option<Arc<dyn TryAfterToolCallHook>>,
    ) -> Result<(), AgentError> {
        self.inner
            .set_try_after_tool_call_object(
                hook.map(|h| {
                    Arc::new(TryAfterToolCallAdapter(h)) as Arc<dyn agent::TryAfterToolCall>
                }),
            )
            .map_err(AgentError::from)
    }
    ...
    pub fn set_should_stop_after_turn_hook(
        &self,
        hook: Option<Arc<dyn ShouldStopAfterTurnHook>>,
    ) -> Result<(), AgentError> {
        self.inner
            .set_should_stop_after_turn_object(
                hook.map(|h| Arc::new(ShouldStopAdapter(h)) as Arc<dyn agent::ShouldStopAfterTurn>),
            )
            .map_err(AgentError::from)
    }

    /// Set (or clear) the prepare-next-turn hook.
    pub fn set_prepare_next_turn_hook(
        &self,
        hook: Option<Arc<dyn PrepareNextTurnHook>>,
    ) -> Result<(), AgentError> {
        self.inner
            .set_prepare_next_turn_object(
                hook.map(|h| {
                    Arc::new(PrepareNextTurnAdapter(h)) as Arc<dyn agent::PrepareNextTurn>
                }),
            )
            .map_err(AgentError::from)
    }
    ...
    pub fn set_steering_source(
        &self,
        source: Option<Arc<dyn QueueMessageSource>>,
    ) -> Result<(), AgentError> {
        self.inner
            .set_steering_source_object(
                source.map(|s| Arc::new(QueueSourceAdapter(s)) as Arc<dyn agent::QueueSource>),
            )
            .map_err(AgentError::from)
    }
...
mod tests {
    ...
    async fn prompt_drives_typed_sink_events() {
        ...
        assert!(
            saw_text,
            "expected a MessageUpdate carrying the scripted text; events: {:?}",
            events.len()
        );
        drop(events);
    ...
    fn api_keys_constructor_builds() {
        let keys: HashMap<String, String> = [("openai".to_string(), "sk-test".to_string())]
            .into_iter()
            .collect();
    ...
    impl AgentTool for WeatherTool {
        ...
        async fn execute(
            &self,
            call: ToolCallContext,
            _cancel: Arc<AgentCancelToken>,
        ) -> Result<AgentToolResult, AgentError> {
            self.executed.store(true, Ordering::SeqCst);
            assert_eq!(call.tool_name, "get_weather");
            assert!(
                call.args_json.contains("Lisbon"),
                "args_json: {}",
                call.args_json
            );
            Ok(AgentToolResult {
    ...
    fn tool_call_setup() -> (AgentSetup, Arc<MockStreamFn>) {
        let tool_call = agent::AgentToolCall::new(
            "call-1",
            "get_weather",
    ...
    async fn host_tool_executes_and_streams_events() {
        ...
        agent
            .prompt("weather in Lisbon?".to_string())
            .await
            .unwrap();

        assert!(
            tool.executed.load(Ordering::SeqCst),
            "host tool must execute"
        );
        let events = sink.0.lock().unwrap();
        ...
        assert!(
            saw_tool_end,
            "expected ToolExecutionEnd with the host tool's result"
        );
    }
    ...
    async fn steering_source_injects_message_into_run() {
        ...
        assert!(
            transcript_has("steer me"),
            "steered message should be in the transcript"
        );
        assert!(
            transcript_has("answer after steering"),
            "loop should continue after steering"
        );
    }
    ...
    async fn cleared_hook_no_longer_blocks() {
        ...
        mock.push_stream(agent::testing::ScriptedStream::from_message(
            fixtures::tool_use_msg(vec![agent::AgentToolCall::new(
                "call-2",
                "get_weather",
        ...
        mock.push_stream(agent::testing::ScriptedStream::from_message(
            fixtures::text_msg("done"),
        ));
        agent.prompt("weather in Porto?".to_string()).await.unwrap();
        assert!(
            tool.executed.load(Ordering::SeqCst),
            "cleared hook: tool must execute"
        );
    }
