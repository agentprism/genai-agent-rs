...
> Correction: Pinned Pi's non-streaming OpenRouter Images adapter places the complete OpenAI SDK `.withResponse()` operation inside `retryProviderRequest`, but that does not make every pre-semantic failure retryable. It invokes the SDK request closure when the supplied abort signal is already aborted, but OpenAI SDK 6.40.0 rejects that operation before invoking the injected `fetch`; no transport attempt occurs. The SDK propagates successful-response body-read and JSON-decoding failures as ordinary errors without the `status` and `headers` properties required by Pi's `isProviderError` predicate, so they receive exactly one request even when `maxRetries` is positive. For a non-success response, however, the SDK catches a body-read failure while constructing an `APIError` and retains the response status and headers; Pi's outer retry helper therefore still applies HTTP retry classification. The SDK maps status 204 to `null` without reading the body, maps a JSON response with an exact `Content-Length: 0` to `undefined` without reading the body, and otherwise parses only `application/json`/`+json` media types as JSON, returning response text for other content types. JSON parsing uses Fetch's replacement-decoded UTF-8 text. Pi invokes `onResponse` after that successful SDK parsing and before projecting the parsed value into `AssistantImages`; if projection then fails, the catch path retains response ID, usage, and content already assigned to the preallocated result. The Rust Send and Local adapters preserve those retry-eligibility, parsing, observer-order, and partial-projection boundaries while keeping the SDK-equivalent nested retries disabled (`packages/ai/src/api/openrouter-images.ts:62–95`; `packages/ai/src/utils/provider-retry.ts:8–25,103–129`; OpenAI SDK 6.40.0 `client.ts` non-success handling and `internal/parse.ts` response parsing).
...
> Correction: Pinned OpenRouter Images is a family-specific exception to the general logical-header order above. `ImagesModels.generateImages` merges resolved auth headers before explicit option headers, then `openrouter-images.ts` overlays that merged map after model headers when constructing the OpenAI client's defaults; the separately supplied `apiKey` establishes request eligibility. Rust Send and Local image dispatch therefore order provider defaults → model headers → resolved auth headers → explicit headers → final transforms, while retaining the resolved API key separately. A model, explicit, or transformed `Authorization` header can affect the final wire header but cannot make an otherwise keyless request eligible (`packages/ai/src/images-models.ts:183–212`; `packages/ai/src/api/openrouter-images.ts:40–60,118–129`).
