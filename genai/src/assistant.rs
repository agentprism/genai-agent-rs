...
/// Internal helper (hidden from the public API): current wall-clock time in epoch milliseconds,
/// saturating at `i64::MAX`. Re-exported by the agent crate for its message constructors.
#[doc(hidden)]
...
/// Internal helper (hidden from the public API): the placeholder model identity used when a real
/// one is unavailable. Re-exported by the agent crate for its default configuration.
#[doc(hidden)]
pub fn unknown_model_iden() -> ModelIden {
