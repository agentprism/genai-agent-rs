...
//! stream, exactly as the [`genai::StreamFn`] contract requires. The
//! only fallible boundary that surfaces a `Result` is the [`crate::codex::TokenSource`],
//! and even that error is converted to an in-band terminal event by
//! [`crate::codex::CodexStreamFn`]. This enum therefore exists for token-source
//! implementations and callers that want a typed error.
//!
//! [`AssistantMessageEvent::Error`]: genai::AssistantMessageEvent::Error

/// An error obtaining a fresh Codex bearer + account id from a [`crate::codex::TokenSource`].
