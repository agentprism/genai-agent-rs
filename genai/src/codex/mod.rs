...
//!
//! - [`genai`] provides the [`StreamFn`](genai::StreamFn)
//!   trait and the `AssistantMessageEvent` protocol every stream function emits,
//!   but knows nothing about auth.
//! - [`genai::auth`] owns the ChatGPT Codex OAuth flow, token cache, and
...
//! ```
//!
//! With the `codex-auth-resolver` feature, build the token source from the auth crate's
