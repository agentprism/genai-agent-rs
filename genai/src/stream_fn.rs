...
use crate::{
    ...
    PriceCatalog, StopReason, compute_cost,
};
...
///
/// Hidden from the public API: it is an internal helper consumed by the agent crate's run
/// admission path (re-exported there), not part of the supported surface.
#[doc(hidden)]
pub fn get_default_stream_fn() -> Option<Arc<dyn StreamFn>> {
    default_slot()
        .read()
        .unwrap_or_else(std::sync::PoisonError::into_inner)
        .clone()
...
#[cfg(feature = "testing")]
#[doc(hidden)]
pub fn stream_error_model(model: &ModelSpec) -> ModelIden {
