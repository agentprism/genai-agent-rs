...
- `!` Fork packaging: merged upstream `0.7.0-beta.20-WIP` (`9524ec9..bd84fbb`); the fork version is re-derived to `0.7.0-beta.20-WIP.1-agentprism`. The fork's streaming HTTP error-headers change is now an upstream feature (PR #280), so upstream's entry below records it.
- `+` `Client` adds request-scoped exec-hook overrides: `exec_chat_with_exec_options` and `exec_chat_stream_with_exec_options` accept an optional `&ExecOptions` (`#[non_exhaustive]`, built via `ExecOptions::new()` and the `with_*` / `without_*` builders). Each channel — the `PayloadInterceptor` and the `ResponseObserver` — independently resolves to exactly one state per HTTP attempt via `ExecHookOverride`: `Inherit` (the construction-time hook, the default), `Replace` (the request hook instead of the construction-time hook), or `Disable` (no hook for this request). At most one hook fires per channel per attempt, and a request replacement never composes with the construction-time default. The classic `exec_chat` / `exec_chat_stream` delegate with an all-`Inherit` value, byte-identical to the pre-override behavior.
- `!` Fork packaging: the package version is now `0.7.0-beta.19.2-agentprism` (fork lineage on top of upstream's 0.7.0-beta.19 WIP line) and `repository` points at the agentprism fork, keeping the fork artifact distinct from — and predictably ordered against — anything upstream may publish (beta.19 < this < beta.20). The fork is published to crates.io as `genai-agentprism` (lib target still named `genai`, so dependents keep writing `use genai::…` via `package = "genai-agentprism"`).
- `!` API CHANGE - `AgentToolCall` adds the public `namespace: Option<String>` field (pi-ai `ToolCall.namespace`, e.g. OpenAI Responses namespaced tools) with a `with_namespace` builder. Downstream `AgentToolCall` struct literals must add `namespace: None`, or preferably migrate to `AgentToolCall::new(...)` and the builder methods. `From<genai::chat::ToolCall>` maps it to `None` (upstream genai carries no namespace).
