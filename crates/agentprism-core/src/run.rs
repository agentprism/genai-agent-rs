...
pub(crate) struct AgentDefaults {
    ...
    model: agentprism_ai::ModelRef,
    reasoning: agentprism_ai::ReasoningLevel,
    tools: ToolRegistry,
}
...
impl LocalAgent {
    ...
    pub fn new(
        runtime: Rc<dyn agentprism_ai::LocalModelRuntime>,
        state: AgentState,
        tools: LocalToolRegistry,
    ) -> Result<Self, AgentError> {
        tools.validate()?;
        ...
        async_stream::stream! {
            ...
            loop {
                ...
                default_options.reasoning = match current_reasoning {
                    agentprism_ai::ReasoningLevel::Off => None,
...
impl crate::Agent {
    ...
    async fn prepare_context(
        ...
        let context = Context {
            schema_version: agentprism_ai::CONTEXT_SCHEMA_VERSION,
...
fn provider_neutral_handoff_report(
    model: &agentprism_ai::ModelRef,
    messages: &[agentprism_ai::Message],
) -> agentprism_ai::HandoffReport {
    let mut report = agentprism_ai::HandoffReport::unchanged(agentprism_ai::ModelFingerprint::new(
        model.provider.clone(),
        agentprism_ai::ApiId::new("provider-neutral"),
        model.model.clone(),
    ));
    for message in messages {
        if let agentprism_ai::Message::Assistant(assistant) = message {
            report
                .source_models
                .insert(agentprism_ai::ModelFingerprint::new(
...
fn add_cost(
    total: Option<agentprism_ai::Cost>,
    next: Option<&agentprism_ai::Cost>,
) -> Result<Option<agentprism_ai::Cost>, ()> {
...
fn apply_next_turn<Tools>(
    ...
    model: &mut agentprism_ai::ModelRef,
    reasoning: &mut agentprism_ai::ReasoningLevel,
) {
