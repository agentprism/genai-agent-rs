...
impl TokioAgentHandle {
    ...
    pub async fn steer(
        &self,
        message: AgentRecord,
    ) -> Result<agentprism_core::QueueReceipt, agentprism_core::ControlError> {
        let (response, receiver) = oneshot::channel();
        self.command_tx
            .send(AgentCommand::Steer { message, response })
            .await
            .map_err(|_| agentprism_core::ControlError::Closed)?;
        receiver
            .await
            .map_err(|_| agentprism_core::ControlError::Closed)?
    }
    ...
    pub async fn cancel(&self, run_id: RunId) -> Result<(), agentprism_core::ControlError> {
        let (response, receiver) = oneshot::channel();
        self.command_tx
    ...
    pub fn cancel_now(&self, run_id: RunId) -> Result<(), agentprism_core::ControlError> {
...
enum AgentCommand {
    ...
    Steer {
        message: AgentRecord,
        response:
            oneshot::Sender<Result<agentprism_core::QueueReceipt, agentprism_core::ControlError>>,
    },
    FollowUp {
        message: AgentRecord,
        response:
    ...
    Cancel {
        run_id: RunId,
        response: oneshot::Sender<Result<(), agentprism_core::ControlError>>,
    },
    ResetTranscript {
...
fn apply_event_to_snapshot(
    ...
    match event {
        ...
        AgentEvent::MessageCommitted { message } => {
            ...
            if matches!(
                message,
                AgentRecord::Llm(agentprism_ai::Message::Assistant(_))
            ) {
