...
async fn restore_pending_core_queues(
    state: &SessionState,
    lane: &pi_agent_session::LaneName,
...
struct PendingQueueItem {
    sequence: pi_agent_session::Sequence,
    run_id: Option<RunId>,
    queue: QueueKind,
...
fn validate_recovery_log(
    ...
    let mut starts = BTreeMap::<RunId, pi_agent_session::Sequence>::new();
    let mut finished = BTreeMap::<RunId, pi_agent_session::Sequence>::new();
    let mut aborted = BTreeMap::<RunId, pi_agent_session::Sequence>::new();
    let mut attempts = BTreeMap::<RunId, RecoveryAttempt>::new();
    let mut enqueued = BTreeMap::<(Option<RunId>, EntryId), pi_agent_session::Sequence>::new();
...
fn reconstruct_run_state(
    ...
    for record in records {
        ...
        let belongs_to_run = matches!(
            attribution,
            pi_agent_session::UsageAttribution::Assistant {
                run_id: owner, ..
            } | pi_agent_session::UsageAttribution::DeferredFetch {
                run_id: owner, ..
            } if owner == run_id
        );
        if !belongs_to_run {
...
fn derive_run_recovery_plan(
    ...
    let own_entries = state
        ...
        .transpose()
        .map_err(pi_agent_session::SessionError::from)?
        .unwrap_or_default()
        .into_iter()
        ...
        )
    {
        ...
        let produced_by_deferred_fetch = completed_steps.iter().any(|record| {
            matches!(
                record,
                OperationRecord::Usage {
                    attribution: pi_agent_session::UsageAttribution::DeferredFetch { entry_id, .. },
                    ..
