...
impl LocalSessionStorage for RcOnlyStorage {
    fn metadata(
        &self,
    ) -> LocalBoxFuture<'_, Result<pi_agent_session::SessionMetadata, pi_agent_session::SessionError>>
    {
    ...
    fn load_state(
        &self,
    ) -> LocalBoxFuture<'_, Result<pi_agent_session::SessionState, pi_agent_session::SessionError>>
    {
    ...
    ) -> LocalBoxFuture<'_, Result<pi_agent_session::AppendReceipt, pi_agent_session::SessionError>>
    {
    ...
    fn log(
        ...
        limit: Option<usize>,
    ) -> LocalBoxFuture<'_, Result<Vec<SessionMutation>, pi_agent_session::SessionError>> {
    ...
    fn repair_tail(
        ...
        '_,
        Result<pi_agent_session::TailRepairReport, pi_agent_session::SessionError>,
    > {
        ...
        .append_batch(
            ...
            vec![SessionMutation::Entry {
                ...
                entry: SessionEntry::Custom {
                    ...
                    data: Some(pi_ai::VersionedExtension {
                        schema_version: 1,
...
struct RecordingRuntime {
    scripted: pi_ai::ScriptedRuntime,
    requests: Mutex<Vec<ModelRequest>>,
}

impl RecordingRuntime {
    fn new(responses: impl IntoIterator<Item = pi_ai::ScriptedResponse>) -> Self {
        Self {
            scripted: pi_ai::ScriptedRuntime::new(responses),
...
type ReasoningObservations =
    Arc<Mutex<Vec<(pi_ai::ReasoningLevel, Option<pi_ai::ReasoningLevel>)>>>;
...
impl AuthoritativeOptionsContextPolicy {
    fn prepare(&self, state: AgentStateView<'_>) -> PreparedAgentRecords {
        ...
        options.reasoning = Some(pi_ai::ReasoningLevel::Low);
...
fn compaction_threshold_decision() {
    ...
    let runtime = Arc::new(pi_ai::ScriptedRuntime::new([]));
...
fn compaction_manual_reason() {
    ...
    block_on(async {
        ...
        assert!(
            pi_agent_core::LocalContextPolicy::prepare_agent_records(
                &local_context_policy(
...
fn compaction_projection_empty_nonempty_path_invokes_summary_send() {
    ...
    block_on(async {
        ...
        let runtime = Arc::new(RecordingRuntime::new([pi_ai::text_response(
...
fn compaction_projection_empty_nonempty_path_invokes_summary_local() {
    ...
    block_on(async {
        ...
        let runtime = Rc::new(RecordingRuntime::new([pi_ai::text_response(
...
fn compaction_retains_configured_tail() {
    ...
    block_on(async {
        ...
        let runtime = Arc::new(RecordingRuntime::new([
            pi_ai::text_response("history"),
            pi_ai::text_response("bash prefix"),
        ]));
        let policy = RuntimeCompactionPolicy::new(
            runtime.clone(),
            ModelRef::new("test", "summary"),
        ...
        let structural_runtime = Arc::new(RecordingRuntime::new([pi_ai::text_response(
            "structural history",
        )]));
        ...
        let structural_compacted = structural_policy
            .compact(
                CompactionInput {
                    ...
                    custom_instructions: None,
                    reasoning: pi_ai::ReasoningLevel::Off,
                    timestamp: Timestamp::from_unix_millis(10),
                },
                CancellationToken::new(),
            )
    ...
    block_on(async {
        ...
        let result = policy
            .compact(
                CompactionInput {
                    ...
                    reasoning: pi_ai::ReasoningLevel::Medium,
        ...
        );
        {
            ...
            assert_eq!(
                ...
                Some(pi_ai::ReasoningLevel::Medium)
            );
            assert_eq!(
        ...
        let empty_runtime = Arc::new(RecordingRuntime::new([pi_ai::text_response(
        ...
        let split_runtime = Arc::new(RecordingRuntime::new([
            pi_ai::text_response("history summary").with_usage(zero_total),
            pi_ai::text_response("turn prefix summary").with_usage(reported_total),
        ]));
...
fn compaction_context_uses_latest_compaction_entry() {
    ...
    block_on(async {
        ...
        let summary_runtime = Arc::new(RecordingRuntime::new([pi_ai::text_response(
        ...
        assert_eq!(high_options.reasoning, Some(pi_ai::ReasoningLevel::High));
        ...
        assert_eq!(
            ...
            Some(pi_ai::ReasoningLevel::High)
        );
...
fn branch_summary_zero_context_window_uses_pi_fallback_budget() {
    ...
    let send = RuntimeBranchSummaryPolicy::new(
        Arc::new(pi_ai::ScriptedRuntime::new([])),
        ModelRef::new("test", "summary"),
        0,
    );
    let local = LocalRuntimeBranchSummaryPolicy::new(
        Rc::new(pi_ai::ScriptedRuntime::new([])),
        ModelRef::new("test", "summary"),
        0,
    );
...
fn branch_summary_summarizes_abandoned_segment() {
    ...
    block_on(async {
        ...
        let empty_instruction_runtime = Arc::new(RecordingRuntime::new([
            pi_ai::text_response("default prompt with append mode"),
            pi_ai::text_response("default prompt with replace mode"),
        ]));
        ...
        let budget_runtime = Arc::new(RecordingRuntime::new([pi_ai::text_response(
