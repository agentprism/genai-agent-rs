...
use agentprism_session::{CompactionReason, EntryId, SessionEntry};
...
pub struct CompactionResult {
    ...
    pub cost: Option<agentprism_ai::Cost>,
...
pub fn estimate_record_tokens(record: &AgentRecord) -> Result<u64, CompactionError> {
    match record {
        ...
        AgentRecord::Llm(_) | AgentRecord::Custom { .. } => project_record_to_message(record, 0)
            .map_or(Ok(0), |message| {
                agentprism_ai::estimate_message_tokens(&message).map_err(|error| {
...
///
/// Unlike `agentprism_ai` request planning, harness compaction considers messages only:
...
fn combine_terminal_messages(
    ...
    combined.cost = match (&history.cost, &prefix.cost) {
        (Some(left), Some(right)) if left.currency == right.currency => Some(agentprism_ai::Cost {
...
fn format_tool_arguments(call: &agentprism_ai::ToolCall) -> String {
    ...
    arguments
        .iter()
        .map(|(key, value)| {
            let value = agentprism_ai::json_stringify_compatible(value)
