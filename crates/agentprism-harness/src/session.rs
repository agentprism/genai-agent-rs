...
use agentprism_env::{Clock, LocalClock};
...
impl Session {
    ...
    pub(crate) async fn commit_agent_record(
        ...
        run_id: RunId,
        entry_id: EntryId,
        message: agentprism_core::AgentRecord,
        terminate: bool,
    ) -> Result<AppendReceipt, SessionError> {
        let _guard = self.mutation_lock.lock().await;
        let state = self.storage.load_state().await?;
        ...
        if let agentprism_core::AgentRecord::Llm(Message::ToolResult(result)) = &message
    ...
    pub(crate) async fn commit_assistant(
        ...
        let entry = SessionMutation::Entry {
            ...
            entry: SessionEntry::Message {
                ...
                message: agentprism_core::AgentRecord::Llm(Message::Assistant(message)),
                terminate: false,
            },
        };
        self.storage
    ...
    pub(crate) async fn commit_compaction(
        ...
        entry_id: EntryId,
        summary: String,
        retained_tail: Vec<agentprism_core::AgentRecord>,
        tokens_before: u64,
