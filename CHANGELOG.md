...
- `!` Fork packaging: the package version is now `0.7.0-beta.19.1-agentprism` (fork lineage on top of upstream's 0.7.0-beta.19 WIP line) and `repository` points at the agentprism fork, keeping the locally packaged archive distinct from anything upstream may publish. Distribution of the fork is via locally produced crate archives only; this version is not on crates.io.
