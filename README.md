...
> merges. The crate uses APIs that PR introduces (for example `ToolResponse::parts`, and the
> client-level `PayloadInterceptor`/`ResponseObserver` exec hooks backing `on_payload`/`on_response`),
> so a plain crates.io `genai` version field cannot resolve them.
...
|---|---|
| Mapped behavior | The pinned non-harness matrix is 55/55 green with no mapped divergence. The M1-M6 checkpoint contained 112 green tests; release-hardening regressions and upstream/production-parity additions (including the `on_payload`/`on_response` exec hooks) bring the current all-feature suite to 163/163. |
