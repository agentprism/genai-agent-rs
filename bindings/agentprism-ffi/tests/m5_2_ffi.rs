...
struct ReentrantCancellationState {
    agent: *mut agentprism_ffi::c_api::pi_agent_handle,
    run_id: u64,
    envelopes: Mutex<Vec<Value>>,
    terminal: Condvar,
...
fn begin_callback_manual_session(
    models: &PiModels,
    manual_paste: bool,
) -> std::sync::Arc<agentprism_ffi::PiAuthSession> {
    models
        .auth_login_begin(
...
fn read_shared_challenge(session: &agentprism_ffi::PiAuthSession, also_accepts_manual_code: bool) {
...
fn assert_auth_session_completes(session: &agentprism_ffi::PiAuthSession) {
